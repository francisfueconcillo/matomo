 ...
 *
 */
namespace Piwik\Plugins\MiscWidgets\Widgets;
...
class GetCurrentTime extends Widget
{
    public static function configure(WidgetConfig $config)
    {
         ...
         * your own category.
         */
        $config->setCategoryId('MiscWidgets_AboutMatomo');
         ...
         */
        $config->setName('MiscWidgets_CurrentTime');
        ...
        $idSite = Request::getRequestParametersGET()['idSite'];

        if (empty($idSite)) {
            throw new Exception('MiscWidgets_CurrentTime -  Site dont have a default timezone.');
        } else {
            $site  = new Site($idSite); 
            $config->addParameters(['sitetz' => $site->getTimezone()]);
        }
    ...
    {
        $params = Request::getRequestParametersGET();

        if(isset($params['sitetz'])){
            return $this->renderTemplate('currentTime', array(
                'sitetz' => urldecode($params['sitetz'])
            ));
        } else {
            throw new Exception('MiscWidgets_CurrentTime -  Site dont have a default timezone.');
        }
